...
async function checkUserInGroup(uid,groupname){
    return new Promise((resolve, reject) => { // Make the function async
        ldap.connectToLDAPServer((err, client) => {
            ...
            const searchOptions = {
                filter: `(&(cn=${groupname})(member=uid=${uid},ou=people,dc=example,dc=com))`,
                scope: 'sub',
                attributes: ['member'],
            };
            ...
            client.search('ou=group,dc=example,dc=com', searchOptions, async (err, res) => {
                ...
                res.on('end', ()=>{
                    console.log('User not belong to this group');
                    client.unbind();
                    resolve(false)
                })
            });
...
function addRole(roleName,members){
    return new Promise((resolve, reject) => {
        const membersDN = []
        for (const member of members){
            membersDN.push(`uid=${member},ou=people,dc=example,dc=com`)
        }
