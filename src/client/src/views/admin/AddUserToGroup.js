...
const AddRoleForUSer = () => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
        try {
            const response = await axios.get(`http://localhost:9000/api/admin/getUserGroupToModify/${uid}`,
                {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      window.location.href = `http://localhost:3000/admin/manage-groups/detail/${uid}`;
    }catch (error){
  ...
  return (
    <div className="container mx-auto mt-8">
      <h1 className="text-2xl font-semibold mb-4">Add New Role For User</h1>
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <div className='flex justify-end'>
            ...
            >
            <Link to={`/admin/manage-groups/detail/${uid}`}>
              Cancel
            </Link>
            </button>
          </div>
